...
st.title("📈 关键词全周期表现汇总")
st.markdown("已统一指标格式：**目标指标**、**真实ROAS**、**真实点击率** 均以百分比展示。")
...
if uploaded_files:
    ...
    if all_data:
        ...
        # --- 界面展示与格式化 ---
        st.success(f"✅ 汇总完成！已分析 {len(keyword_summary)} 个关键词。")

        # 使用 Streamlit 的 column_config 进行百分比美化显示
        ...
        # 下载部分（CSV 保持纯数字方便二次计算）
        final_csv = keyword_summary.to_csv(index=False).encode('utf-8-sig')
        st.download_button("📥 下载汇总报表 (原始数据)", final_csv, "LxU_Keyword_Analysis.csv", "text/csv")
