...
if uploaded_files:
    ...
    if all_data:
        ...
        with tab1:
            ...
            # --- 列顺序调整：支出占比 移至 维度 前，点击率 移至 转化率 后 ---
            st.dataframe(t1_df.style.apply(lambda r: apply_lxu_style(r, True), axis=1), 
                         ...
                         column_order=("产品编号", "支出占比", "维度", "目标指标", "真实ROAS", "转化率", "点击率", "展示量", "点击量", "真实CPC", "广告真实支出", "销售额"),
        ...
        with tab2:
            ...
            # --- 列顺序调整：支出占比 移至 关键词 前，点击率 移至 转化率 后 ---
            st.dataframe(t2_df.style.apply(lambda r: apply_lxu_style(r, False), axis=1), 
                         use_container_width=True, hide_index=True, height=1000,
                         column_order=("产品编号", "支出占比", "维度", "关键词", "策略日期", "目标指标", "真实ROAS", "转化率", "点击率", "展示量", "点击量", "真实CPC", "广告真实支出", "销售额"),
                         column_config=common_config)
